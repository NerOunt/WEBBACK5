...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (!empty($_COOKIE['login_credentials'])) {
        ...
        $messages[] = '<div class="credentials-message"><h3>Ваши данные для входа:</h3>
                     <p><strong>Логин:</strong> '.htmlspecialchars($credentials['login']).'</p>
                     <p><strong>Пароль:</strong> '.htmlspecialchars($credentials['password']).'</p>
                     <p class="warning">Сохраните эти данные, они больше не будут показаны!</p></div>';
    ...
    if ($logged_in) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT a.* FROM applications a JOIN users u ON a.user_id = u.id WHERE u.id = ?");
            ...
            if ($user_data) {
                ...
                $stmt = $pdo->prepare("SELECT pl.id FROM programming_languages pl 
            ...
            $messages[] = '<div class="error">Ошибка базы данных: '.htmlspecialchars($e->getMessage()).'</div>';
        }
    } else {
        ...
        try {
            ...
            if ($logged_in) {
                ...
                try {
                    ...
                    setcookie('login_credentials', json_encode([
                        'login' => $login,
                        'password' => $password
                    ]), time() + 24 * 60 * 60);
